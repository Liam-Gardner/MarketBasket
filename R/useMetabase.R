...
path_data_csv_1 <- capture.output(cat("rules", path_data_csv, sep = '/'))
retail <- read.csv(file = path_data_csv_1)
...
path_mba_1 <- capture.output(cat("rules", path_mba, sep = '/'))
write.csv(itemList, path_mba_1, quote = FALSE, row.names = TRUE)
...
tr <- read.transactions(path_mba_1, format = 'basket', sep = ',', rm.duplicates = TRUE, quote = "")
...
path_rulesJson_1 <- capture.output(cat("rules", path_rulesJson, sep = '/'))
write(rules_json, file = path_rulesJson_1)
